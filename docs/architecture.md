...
The project follows a hexagonal architecture with clear boundaries:
...
Equivalent adapter packages should follow the same boundary rules for Vue, Preact, and Astro.
...
This package should not depend on React.
It now includes a framework-free browser runtime (`createCommandPalette`) for HTML/CSS/JS integration on top of the headless engine.
...
- styling hooks via `classNames` and runtime CSS variables
...
Official framework packages mirror the same adapter role:

- `packages/react`
- `packages/vue`
- `packages/preact`
- `packages/astro`
- framework-free runtime directly in `packages/core` (`createCommandPalette`)
...
- token-based theming (single mode and dual mode `light`/`dark`)
- class hooks for adapter-level CSS overrides
- predictable runtime CSS variable generation from `@cmd-kit/core` helpers

The theme model remains token-based so playground exports map directly to package API.
